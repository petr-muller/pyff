...
def test_FullyQualifyNames_import():  # pylint: disable=invalid-name
    ...
    _check_fqn("import os.path as pathy", "path = pathy", {"pathy": "os.path"}, "path = os.path")
...
def test_find_external_name_matches_import():  # pylint: disable=invalid-name
    ...
    assert (
        ps.find_external_name_matches(parse_pkg(), parse_pkg(), package_imports, package_imports)
        is None
    )
