...
class TestImportedName:
    ...
    def test_fqast_imports(self):
        ...
        alias = ast.alias(name='os.path', asname="path")
        alias_ast = ast.dump(pi.ImportedName('path', ast.Import(names=[alias]),
                                             alias).canonical_ast)
    ...
    def test_fqdn_importfrom(self):
        ...
        assert (pi.ImportedName('four', alias_name, alias).canonical_name ==
    ...
    def test_fqast_importfrom(self):
        ...
        module_ast = ast.dump(module_name.canonical_ast)
        assert module_ast == "Attribute(value=Attribute(value=Attribute(value=Name(id='one', ctx=Load()), attr='two', ctx=Load()), attr='three', ctx=Load()), attr='four', ctx=Load())" # pylint: disable=line-too-long
        ...
        assert alias_ast == "Attribute(value=Attribute(value=Attribute(value=Name(id='one', ctx=Load()), attr='two', ctx=Load()), attr='three', ctx=Load()), attr='fourth_module', ctx=Load())" # pylint: disable=line-too-long
...
class TestImportedNames:
    ...
    def test_importfrom(self):
        names = pi.ImportedNames()
        names.add_importfrom(ast.ImportFrom(module='os', level=0,
                                            names=[ast.alias(name='path', asname=None),
                                                   ast.alias(name='environ', asname=None)]))
        names.add_importfrom(ast.ImportFrom(module='sys', level=0,
                                            names=[ast.alias(name='exit', asname=None)]))
        assert len(names) == 3
...
class TestImportsPyfference:
    ...
    def test_message_new_import(self):
        ...
        assert (str(pi.ImportedNames.compare(old, new_more)) ==
    ...
    def test_message_remove_fromimport(self):
        ...
        assert (str(pi.ImportedNames.compare(old, new_one)) ==
    ...
    def test_message_removed_fromimport_module(self):  # pylint: disable=invalid-name
        old = parse_imports("from module import one")
        new = parse_imports("")
        assert (str(pi.ImportedNames.compare(old, new)) ==
